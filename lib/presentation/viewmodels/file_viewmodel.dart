...
class FileViewModel extends StateNotifier<List<FileInfoState>> {
  ...
  FileViewModel(this.uploadFileUseCase, this.uploadNotifier, this.getFileListUseCase) : super([]) {
    // 초기 파일 목록 로드
    refreshFileList();
  }
  ...
  Future<void> refreshFileList() async {
    ...
    print(fileList);
    state = fileList.map((file) => FileInfoState(
...
final fileViewModelProvider = StateNotifierProvider<FileViewModel, List<FileInfoState>>((ref) {
  ...
  return FileViewModel(uploadUseCase, uploadNotifier, getFileListUseCase);
});